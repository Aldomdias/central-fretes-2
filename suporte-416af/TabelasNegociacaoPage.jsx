...
<body>
  ...
  async function handleExcluirRegistroRodada(rodada) {
    ...
    setSalvando(true);
    setErro('');
    setSucesso('Excluindo registro da rodada...');
  ...
  }  async function handleLimparRodadasVazias() {
    if (!selecionada) return;

    var historico = getHistoricoRodadasTabela(selecionada);
    var rodadasComSimulacao = {};

    historico.forEach(function(item) {
      if (String(item.tipo_registro || '').toUpperCase() === 'SIMULACAO') {
        rodadasComSimulacao[Number(item.rodada || 0)] = true;
      }
    });

    var candidatos = historico.filter(function(item) {
      var tipo = String(item.tipo_registro || '').toUpperCase();
      var rodada = Number(item.rodada || 0);
      var imp = item.itens_importados || {};
      var salvos = item.itens_salvos_apos_importacao || {};
      var total =
        Number(imp.total || 0) +
        Number(imp.rotas || 0) +
        Number(imp.cotacoes || 0) +
        Number(salvos.total || 0) +
        Number(salvos.rotas || 0) +
        Number(salvos.cotacoes || 0);

      return rodada > 0 && rodadasComSimulacao[rodada] && tipo !== 'SIMULACAO' && total === 0;
    });

    if (!candidatos.length) {
      setSucesso('Nenhuma rodada vazia encontrada para limpar.');
      return;
    }

    if (!window.confirm('Remover ' + candidatos.length + ' registro(s) vazio(s) de rodada? As simulações salvas serão mantidas.')) {
      return;
    }

    setSalvando(true);
    setErro('');
    setSucesso('Limpando rodadas vazias...');

    try {
      var atualizada = selecionada;

      for (var i = 0; i < candidatos.length; i += 1) {
        var registroId = String(candidatos[i].id || candidatos[i].criado_em || '');
        if (registroId) {
      ...
      setSucesso('Rodadas vazias removidas. As simulações salvas foram mantidas.');
    } catch (e) {
      setErro(e.message || 'Erro ao limpar rodadas vazias.');
    } finally {
      setSalvando(false);
    }
            ...
            return (
              <div>
                ...
                <div className="sim-actions" style={{ marginBottom: 14 }}>
                  ...
                  <button className="sim-tab" type="button" onClick={handleLimparRodadasVazias} disabled={salvando}>Limpar rodadas vazias</button>
