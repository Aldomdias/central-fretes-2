...
export async function salvarResultadoSimulacaoNegociacao(id, resultado = {}) {
  ...
  const historicoSemAberturaDaRodada = historicoAnterior.filter((item) => {
    const mesmaRodada = inteiro(item?.rodada || 0) === rodadaAtual;
    const ehAbertura = String(item?.tipo_registro || '').toUpperCase() === 'ABERTURA';
    return !(mesmaRodada && ehAbertura);
  });

  const historicoAtualizado = historicoSemAberturaDaRodada.concat([entradaRodada]).slice(-12);
