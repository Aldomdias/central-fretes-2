...
import {
  ...
  prepararAnaliseRotaFaixaParaNegociacao,
  chaveRotaFaixaLaudo,
  detalheCalculoCteTransportador,
  gerarHtmlAjusteRotaFaixa,
  gerarWorkbookAjusteRotaFaixa,
  MAX_ITENS_AMOSTRA_POR_ROTA,
} from '../utils/ajusteRotaFaixaLaudo';
import {
...
function aplicarFiltrosRealizadoQuery(query, filtros) {
  ...
  // Malhas com mais de uma origem (comum em transportadora oficial, ex.: Campinas +
  // Itupeva) precisam de um OR entre os prefixos — usar só origensDb[0] descartava
  // silenciosamente todas as origens além da primeira antes do filtro no cliente.
...
async function buscarRealizadoLocalCtes(filtros = {}, onProgresso = null) {
  ...
  const linhasMapeadas = rows.map(r => ({
    ...
    cubagemTotal: numeroRealizado(pickRealizadoField(r, ['cubagem_total', 'cubagemTotal'])) || 0,
    pesoCubado: numeroRealizado(pickRealizadoField(r, ['peso_cubado', 'pesoCubado'])) || 0,
    raw: r.raw || null,
...
function normalizarOrigensFiltroRealizadoSim(origens = []) {
  ...
  return expandirOrigensTextoOperacionaisSimulador(saida);
}
...
function montarOrigensNormRealizadoSim(filtros = {}) {
  ...
  return expandirOrigensOperacionaisSimulador(set);
}
...
function filtrarRowsPorOrigensRealizado(rows = [], origens = []) {
  const chaves = expandirOrigensOperacionaisSimulador(new Set((origens || []).map((origem) => normalizarChaveSimulador(origem)).filter(Boolean)));
...
async function buscarRealizadoLocalCtesExpandido(filtros = {}, onProgresso = null) {
  ...
  const origensDaOrigemUnica = origemUnica ? expandirOrigensTextoOperacionaisSimulador([origemUnica]) : [];
  const origemExpandida = origensDaOrigemUnica.length > 1;

  return buscarRealizadoLocalCtes({
    ...filtros,
    origem: origemExpandida ? '' : origemUnica,
    origens: origemUnica ? (origemExpandida ? origensDaOrigemUnica : []) : origens,
  }, onProgresso);
}
...
function gerarLaudosEmailRealizado(resultado = {}) {
  ...
  const ehReajuste = resultado.tipoNegociacao === 'REAJUSTE_TABELA_EXISTENTE'
    || resultado.modoNegociacao === 'REAJUSTE'
    || resultado.filtros?.tipoNegociacao === 'REAJUSTE_TABELA_EXISTENTE';
  const impactoProprio = resultado.analiseReajuste?.impactoProprio || {};
  const valorAtualReajuste = Number(impactoProprio.valorAtual || resultado.freteRealizadoRetidoSelecionada || 0);
  const valorNovoReajuste = Number(impactoProprio.valorNovo || resultado.freteSelecionadaRetida || 0);
  const impactoValorReajuste = Number(impactoProprio.impactoValor || (valorNovoReajuste - valorAtualReajuste));
  const impactoPercentualReajuste = Number(impactoProprio.impactoPercentual || (valorAtualReajuste ? (impactoValorReajuste / valorAtualReajuste) * 100 : 0));
  const impactoMensalReajuste = Number(impactoProprio.impactoMensal || (resultado.meses ? impactoValorReajuste / resultado.meses : impactoValorReajuste));
  const impactoAnualReajuste = Number(impactoProprio.impactoAnual || impactoMensalReajuste * 12);
  const coberturaMensal = Number(resultado.faturamentoSelecionadaMes || 0);
  const faturamentoProjetadoNegociacaoPeriodo = Number(
    ehReajuste
      ? (resultado.faturamentoProjetadoNegociacao || resultado.freteSelecionadaGanhadora || 0)
      : (resultado.freteSelecionadaGanhadora || resultado.faturamentoProjetadoNegociacao || 0)
  );
  const faturamentoProjetadoNegociacaoMensal = Number(
    ehReajuste
      ? (resultado.faturamentoProjetadoNegociacaoMes || resultado.faturamentoSelecionadaGanhadoraMes || 0)
      : (resultado.faturamentoSelecionadaGanhadoraMes || resultado.faturamentoProjetadoNegociacaoMes || 0)
  );
  const faturamentoPerdidoPeriodo = Math.max(Number(resultado.freteSelecionada || 0) - faturamentoProjetadoNegociacaoPeriodo, 0);
  const faturamentoPerdidoMensal = Math.max(coberturaMensal - faturamentoProjetadoNegociacaoMensal, 0);
  const savingNegociacaoMensal = Number(resultado.savingProjetadoNegociacaoMes || resultado.savingSelecionadaVsRealMes || 0);
  const savingNegociacaoAnual = Number(resultado.savingProjetadoNegociacaoAno || resultado.savingSelecionadaVsRealAno || 0);
  ...
  const diretoriaPartes = [
    ...
    '',
    'Resumo executivo',
    ehReajuste ? `- Impacto do reajuste na carteira atual: de ${formatMoney(valorAtualReajuste)} para ${formatMoney(valorNovoReajuste)} no período, variação de ${formatMoney(impactoValorReajuste)} (${formatPercent(impactoPercentualReajuste)}).` : '',
    ehReajuste ? `- Impacto projetado: ${formatMoney(impactoMensalReajuste)} por mês e ${formatMoney(impactoAnualReajuste)} em 12 meses.` : '',
    `- O faturamento total das rotas cobertas pela tabela representa ${formatMoney(coberturaMensal)} por mês.`,
    ehReajuste
      ? `- Na disputa geral, o faturamento projetado da negociação, considerando volume retido pela transportadora e volume capturado, é ${formatMoney(faturamentoProjetadoNegociacaoMensal)} por mês.`
      : `- O faturamento projetado da negociação, considerando somente os CT-es ganhos pela tabela, é ${formatMoney(faturamentoProjetadoNegociacaoMensal)} por mês.`,
    `- O faturamento que permanece fora da captura representa aproximadamente ${formatMoney(faturamentoPerdidoMensal)} por mês dentro do recorte comparado.`,
    `- O saving projetado da negociação é ${formatMoney(savingNegociacaoMensal)} por mês e ${formatMoney(savingNegociacaoAnual)} em 12 meses.`,
    `- A redução média necessária nas rotas perdidas é de ${formatPercent(resultado.reducaoMediaNecessaria)}.`,
    '',
    'Potencial financeiro',
    `- Faturamento projetado da negociação: ${formatMoney(faturamentoProjetadoNegociacaoPeriodo)} no período.`,
    `- Faturamento que perde/não captura: ${formatMoney(faturamentoPerdidoPeriodo)} no período.`,
    `- Saving projetado no período: ${formatMoney(resultado.savingProjetadoNegociacao || resultado.savingSelecionadaVsReal || 0)}.`,
  ...
  return {
    diretoria: {
      ...
      kpis: [
        ...
        ['Faturamento projetado/mês', formatMoney(faturamentoProjetadoNegociacaoMensal)],
        ['Faturamento total rotas/mês', formatMoney(coberturaMensal)],
        ['Faturamento que perde/mês', formatMoney(faturamentoPerdidoMensal)],
        ['Saving/mês', formatMoney(savingNegociacaoMensal)],
...
function numeroRealizado(value) {
  if (typeof value === 'string') {
    ...
    const normalizado = texto.includes(',')
      ...
      : texto;
    const numeroTexto = Number(normalizado.replace(/[^0-9.-]/g, '') || 0);
    return Number.isFinite(numeroTexto) ? numeroTexto : 0;
  }
...
// (compartilhados com a Gestão Base e testados em test/freteCalcEngine.test.js).
...
function enriquecerRealizadoComTracking(rows = [], mapasTracking) {
  ...
  const linhas = (rows || []).map((row) => {
    ...
    const pesoFisico = numeroRealizado(row.peso)
      || numeroRealizado(tracking.peso)
      || numeroRealizado(row.pesoDeclarado)
      || numeroRealizado(tracking.peso_declarado);
    ...
    return {
      ...
      // O peso físico do CT-e prevalece. O Tracking apenas complementa quando o CT-e não informa peso.
      // Isso impede reutilizar peso cubado do Tracking depois que sua cubagem foi rejeitada como outlier.
...
function mesclarTrackingComBase(enriquecidoTracking = {}, enriquecidoBase = {}) {
  const basePorChave = new Map();
  (enriquecidoBase.linhas || []).forEach((row) => {
    const chave = normalizarChaveLongaTracking(row.chaveCte || row.chave_cte || row.numeroCte || row.numero_cte || row.notaFiscal || row.nota_fiscal);
    if (chave) basePorChave.set(chave, row);
  });

  const linhas = (enriquecidoTracking.linhas || []).map((row) => {
    if (row.trackingMatch) return row;
    const chave = normalizarChaveLongaTracking(row.chaveCte || row.chave_cte || row.numeroCte || row.numero_cte || row.notaFiscal || row.nota_fiscal);
    return (chave && basePorChave.get(chave)) || row;
  });
  ...
  return {
    ...
    erroTracking: enriquecidoTracking.erroTracking || enriquecidoBase.erroTracking || '',
  };
}
...
function enriquecerResultadoReajusteNegociacao(resultado = {}, negociacao = {}, transportadoraBase = '', compararConcorrentes = false, compararTabelaAtual = false) {
  ...
  detalhes.forEach((item) => {
    const rota = `${item.origem || '-'}${item.ufOrigem ? `/${item.ufOrigem}` : ''} -> ${item.destino || '-'}${item.ufDestino ? `/${item.ufDestino}` : ''}`;
    const faixa = item.selecionadaDetalhes?.frete?.faixaPeso || item.faixaPeso || '';
    const chave = [rota, faixa, item.canal || ''].join('|');
    const atual = porRota.get(chave) || {
      chave,
      rota,
      origem: item.origem || '',
  ...
  const rotasImpacto = [...porRota.values()].map((item) => {
    const diferenca = item.freteNovo - item.freteAtual;
    return {
      ...item,
      diferenca,
      diferencaPercentual: item.freteAtual ? (diferenca / item.freteAtual) * 100 : 0,
      fretePctAtual: item.valorNF ? (item.freteAtual / item.valorNF) * 100 : 0,
      fretePctNovo: item.valorNF ? (item.freteNovo / item.valorNF) * 100 : 0,
      variacaoGanhos: item.ganhosProjetados - item.ganhosAtuais,
    };
  });

  const rotasAumento = rotasImpacto.filter((item) => item.diferenca > 0.01).sort((a, b) => b.diferenca - a.diferenca);
  const rotasReducao = rotasImpacto.filter((item) => item.diferenca < -0.01).sort((a, b) => a.diferenca - b.diferenca);
  const rotasGanhamCompetitividade = rotasImpacto.filter((item) => item.variacaoGanhos > 0).sort((a, b) => b.variacaoGanhos - a.variacaoGanhos);
  const rotasPerdemCompetitividade = rotasImpacto.filter((item) => item.variacaoGanhos < 0).sort((a, b) => a.variacaoGanhos - b.variacaoGanhos);
  const rotasMantemCompetitividade = rotasImpacto.filter((item) => item.variacaoGanhos === 0 && item.ganhosProjetados > 0);
  const ganhosAtuais = rotasImpacto.reduce((acc, item) => acc + item.ganhosAtuais, 0);
  const ganhosProjetados = rotasImpacto.reduce((acc, item) => acc + item.ganhosProjetados, 0);
  const volumesAtuais = rotasImpacto.filter((item) => item.ganhosAtuais > 0).reduce((acc, item) => acc + item.volumes, 0);
  const volumesProjetados = rotasImpacto.filter((item) => item.ganhosProjetados > 0).reduce((acc, item) => acc + item.volumes, 0);
  const detalhesComRanking = detalhes.filter((item) => Number(item.rankingSelecionada || 0) > 0);
  const rankingMedioNovo = detalhesComRanking.length
    ? detalhesComRanking.reduce((acc, item) => acc + Number(item.rankingSelecionada || 0), 0) / detalhesComRanking.length
    : 0;
  ...
  const analiseReajuste = {
    ...
    impactoProprio: {
      ...
      valorNFAtual: numeroRealizado(resultado.valorNFRetidoSelecionada),
      pesoAtual: numeroRealizado(resultado.pesoRetidoSelecionada),
      volumesAtual: numeroRealizado(resultado.volumesRetidosSelecionada),
      cubagemAtual: numeroRealizado(resultado.cubagemRetidaSelecionada),
      rotasAumento: rotasAumento.slice(0, 30),
      rotasReducao: rotasReducao.slice(0, 30),
      volumetriaImpactada: rotasImpacto.reduce((acc, item) => acc + item.volumes, 0),
    },
    capturaRealizado: {
      ctesCapturados: numeroRealizado(resultado.ctesCapturadosDeOutras),
      freteRealCapturado: numeroRealizado(resultado.freteCapturadoRealizado),
      freteNovaTabela: numeroRealizado(resultado.freteCapturadoTabela),
      savingCapturado: numeroRealizado(resultado.savingCapturado),
      volumesCapturados: numeroRealizado(resultado.volumesCapturados),
      transportadoras: (resultado.transportadorasPerdaDestaque || resultado.impactoTransportadoras || [])
        .filter((item) => numeroRealizado(item.freteCedidoSelecionada) > 0)
        .slice(0, 30),
    },
    competitividade: {
      ganhosAtuais,
      ganhosProjetados,
      rotasGanhas: rotasGanhamCompetitividade.length,
      rotasPerdidas: rotasPerdemCompetitividade.length,
      rotasMantidas: rotasMantemCompetitividade.length,
      aderenciaAtual: qtdAnalisados ? (ganhosAtuais / qtdAnalisados) * 100 : 0,
      aderenciaProjetada: qtdAnalisados ? (ganhosProjetados / qtdAnalisados) * 100 : 0,
      volumesAtuais,
      volumesProjetados,
      freteTotalCompetitivo: numeroRealizado(resultado.freteSelecionadaGanhadora || resultado.freteSelecionada || 0),
      rankingMedioNovo,
      primeirosLugares: detalhesComRanking.filter((item) => Number(item.rankingSelecionada) === 1).length,
      rotasGanhamCompetitividade: rotasGanhamCompetitividade.slice(0, 30),
      rotasPerdemCompetitividade: rotasPerdemCompetitividade.slice(0, 30),
      rotasMantemCompetitividade: rotasMantemCompetitividade.slice(0, 30),
    },
  };
...
function transportadoraCompativelSimulador(nomeTabela = '', nomeFiltro = '') {
  ...
  const tokensTabela = tabela.split(/\s+/).filter(Boolean);
  const tokensFiltro = filtro.split(/\s+/).filter(Boolean);
  if (tokensTabela.some((token) => token.length >= 3 && tokensFiltro.includes(token))) return true;
...
const ORIGEM_DUQUE_CAXIAS_SIM = normalizarChaveSimulador('Duque de Caxias/RJ');
const ORIGEM_RIO_JANEIRO_SIM = normalizarChaveSimulador('Rio de Janeiro/RJ');

function expandirOrigensOperacionaisSimulador(chavesNorm = new Set()) {
  const expandido = new Set(chavesNorm || []);
  if (expandido.has(ORIGEM_DUQUE_CAXIAS_SIM)) {
    expandido.add(ORIGEM_RIO_JANEIRO_SIM);
  }
  return expandido;
}

function origensNormListaSimulador(origens = []) {
  const lista = Array.isArray(origens)
    ? origens
    : String(origens || '').split(',');
  return new Set(lista.map((origem) => normalizarChaveSimulador(origem)).filter(Boolean));
}

function origemOperacionalTabelaSimulador(origemLinha = '', filtros = {}) {
  const origemLinhaNorm = normalizarChaveSimulador(origemLinha);
  if (origemLinhaNorm !== ORIGEM_RIO_JANEIRO_SIM) return '';
  const origensTabelaNorm = origensNormListaSimulador([
    filtros.origem,
    ...(Array.isArray(filtros.origensPadraoTabela) ? filtros.origensPadraoTabela : []),
    ...(Array.isArray(filtros.origens) ? filtros.origens : []),
  ]);
  return origensTabelaNorm.has(ORIGEM_DUQUE_CAXIAS_SIM) ? 'Duque de Caxias/RJ' : '';
}

function expandirOrigensTextoOperacionaisSimulador(origens = []) {
  const saida = [];
  const vistos = new Set();
  ...
  if (vistos.has(ORIGEM_DUQUE_CAXIAS_SIM) && !vistos.has(ORIGEM_RIO_JANEIRO_SIM)) {
    saida.push('Rio de Janeiro/RJ');
  }
  return saida;
}
...
function simularLinhaRealizadoComFallback({ baseOnline = [], row = {}, canal = '', pesoLinha = 0, nf = 0, destino = '', cidadePorIbge, gradeCanal = [], filtros = {}, indicePorDestino }) {
  ...
  const origemOperacionalTabela = origemOperacionalTabelaSimulador(origemLinha, filtros);

  const origensTentativa = valoresUnicosValidos([
    origemLinha,
    origemLinha && ufOrigem ? `${origemLinha}/${ufOrigem}` : '',
    origemOperacionalTabela,
    filtros.origem,
  ]);
  ...
  const origemOperacionalNorm = normalizarChaveSimulador(origemOperacionalTabela);
  const resultadoMesmaOrigem = origemNorm
    ? resultadoDestino.filter((item) => {
        const origemItemNorm = normalizarChaveSimulador(item.origem);
        return origemItemNorm === origemNorm
          || origemItemNorm.includes(origemNorm)
          || origemNorm.includes(origemItemNorm)
          || (origemOperacionalNorm && origemItemNorm === origemOperacionalNorm);
      })
    : resultadoDestino;
...
const CAMPOS_SOMA_RESULTADO_SALVO = [
  ...
  'valorNFComTabelaSelecionada', 'peso', 'volumes', 'cubagemTotal', 'pesoComTabelaSelecionada',
  'volumesComTabelaSelecionada', 'cubagemComTabelaSelecionada', 'linhasComTracking',
...
function consolidarRotasCotacaoRealizado(resultados = []) {
  ...
  (resultados || []).flatMap((item) => item.rotasCotacao || []).forEach((rota = {}) => {
    const nomeRota = String(rota.nome || rota.rota || 'Sem rota/cotacao').trim() || 'Sem rota/cotacao';
    const faixaRota = String(rota.faixaPeso || rota.faixa || '').trim();
    const chave = chaveRotaFaixaLaudo(nomeRota, faixaRota);
...
function consolidarResultadosRealizadoSalvos(analises = []) {
  ...
  base.faturamentoProjetadoNegociacao = base.freteSelecionadaRetida + base.freteCapturadoTabela;
  base.faturamentoProjetadoNegociacaoMes = base.meses ? base.faturamentoProjetadoNegociacao / base.meses : base.faturamentoProjetadoNegociacao;
  base.faturamentoProjetadoNegociacaoAno = base.faturamentoProjetadoNegociacaoMes * 12;
  base.savingProjetadoNegociacao = Math.max((base.freteRealizadoRetidoSelecionada + base.freteCapturadoRealizado) - base.faturamentoProjetadoNegociacao, 0);
  base.savingProjetadoNegociacaoMes = base.meses ? base.savingProjetadoNegociacao / base.meses : base.savingProjetadoNegociacao;
  base.savingProjetadoNegociacaoAno = base.savingProjetadoNegociacaoMes * 12;
  base.impacto_valor = base.freteSelecionadaRetida - base.freteRealizadoRetidoSelecionada;
  base.impacto_percentual = base.freteRealizadoRetidoSelecionada ? (base.impacto_valor / base.freteRealizadoRetidoSelecionada) * 100 : 0;
  base.impacto_mensal = base.meses ? base.impacto_valor / base.meses : base.impacto_valor;
  base.impacto_anual = base.impacto_mensal * 12;
  ...
  base.percentualFreteRealizadoGanharia = base.valorNFGanhariaSelecionada ? (base.freteRealizadoGanhariaSelecionada / base.valorNFGanhariaSelecionada) * 100 : 0;
  base.percentualFreteTabelaGanharia = base.valorNFGanhariaSelecionada ? (base.freteSelecionadaGanhadora / base.valorNFGanhariaSelecionada) * 100 : 0;
  base.variacaoPercentualFreteGanharia = base.percentualFreteRealizadoGanharia ? ((base.percentualFreteTabelaGanharia / base.percentualFreteRealizadoGanharia) - 1) * 100 : 0;
  base.variacaoPercentualFreteComTabela = base.percentualFreteRealizadoComTabela ? ((base.percentualFreteSelecionadaComTabela / base.percentualFreteRealizadoComTabela) - 1) * 100 : 0;
  ...
  base.analiseReajuste = {
    ...(base.analiseReajuste || {}),
    impactoProprio: {
      ...((base.analiseReajuste || {}).impactoProprio || {}),
      valorAtual: base.freteRealizadoRetidoSelecionada,
      valorNovo: base.freteSelecionadaRetida,
      impactoValor: base.impacto_valor,
      impactoPercentual: base.impacto_percentual,
      impactoMensal: base.impacto_mensal,
      impactoAnual: base.impacto_anual,
      ctesAtual: base.ctesRetidosSelecionada,
      valorNFAtual: base.valorNFRetidoSelecionada,
      pesoAtual: base.pesoRetidoSelecionada,
      volumesAtual: base.volumesRetidosSelecionada,
      cubagemAtual: base.cubagemRetidaSelecionada,
    },
    capturaRealizado: {
      ...((base.analiseReajuste || {}).capturaRealizado || {}),
      ctesCapturados: base.ctesCapturadosDeOutras,
      freteRealCapturado: base.freteCapturadoRealizado,
      freteNovaTabela: base.freteCapturadoTabela,
      volumesCapturados: base.volumesCapturados,
    },
  };
...
function serializarEstadoSimulacaoRealizado(estado) {
  ...
  return JSON.stringify({
    ...
    pedidosComTabela: [...(estado.pedidosComTabelaSet || new Set()).values()],
...
async function processarLinhasSimulacaoRealizado(estado, { rows = [], baseOnline = [], baseOnlineAtual = [], transportadoraSelecionada = '', filtros = {}, cidadePorIbge, gradePorCanal = {}, municipioPorCidade, indicePorDestino, onProgress }) {
  ...
  const { rotasMap, cotacaoMap, transportadorasMap, ufsMap, pedidosGanhariaSet, pedidosRetidosSet, pedidosCapturadosSet, pedidosComTabelaSet, pedidosTotaisSet, ctesDetalhes, ctesAjusteRotaExcel, diagnostico } = estado;
  let {
    ctesAnalisados, ctesSimulados, ctesComTabelaSelecionada, ctesGanhariaSelecionada,
    ctesPerdidosSelecionada, ctesSemTabelaSelecionada, ctesSemTabelaGeral, freteRealizado,
    freteRealizadoComTabelaSelecionada, freteSelecionada, freteVencedor, valorNF,
  ...
  for (let indiceLinha = 0; indiceLinha < linhasParaSimular.length; indiceLinha += 1) {
    ...
    if (itemSelecionada) {
      ...
      pesoComTabelaSelecionada += pesoLinha;
      volumesComTabelaSelecionada += vol;
      cubagemComTabelaSelecionada += cubagemLinha;
      pedidosComTabelaSet.add(chavePedido);
    ...
    const deveMedirRetencaoSelecionada = Boolean(
      filtros.compararComProprioRealizado
      || filtros.tipoNegociacao === 'REAJUSTE_TABELA_EXISTENTE'
      || filtros.transportadoraBaseRealizado
    );
    const nomeBaseSelecionadaNorm = normalizarTransportadoraSimulador(filtros.transportadoraBaseRealizado || '');
    const selecionadaJaCarregava = transportadoraCompativelSimulador(row.transportadora || '', transportadoraSelecionada)
      ...
      || (nomeBaseSelecionadaNorm && (
        transportadoraCompativelSimulador(row.transportadora || '', filtros.transportadoraBaseRealizado)
        || normalizarTransportadoraSimulador(row.transportadora || '') === nomeBaseSelecionadaNorm
      ));
    ...
    const nomeCotacao = itemSelecionada?.detalhes?.frete?.rotaNome || itemSelecionada?.rotaNome || 'Sem rota/cotacao';
    const faixaCotacao = itemSelecionada?.detalhes?.frete?.faixaPeso || itemSelecionada?.faixaPeso || '';
    const chaveCotacao = chaveRotaFaixaLaudo(nomeCotacao, faixaCotacao);
    const cotacaoAtual = cotacaoMap.get(chaveCotacao) || {
      nome: nomeCotacao,
      chave: chaveCotacao,
      faixaPeso: faixaCotacao,
      ctes: 0,
      ctesSemCalculo: 0,
      ctesGanharia: 0,
      ctesPerderia: 0,
    ...
    const detalheCteRealizado = {
      ...
      cubagemCorrigidaTracking: Boolean(row.cubagemCorrigidaTracking),
    ...
    const freteSelecionadaDetalhe = Number(detalheCteRealizado.freteSelecionada || detalheTransportadorExcel.tabelaRpa || 0);
    ...
    ctesAjusteRotaExcel.push({
      ...
      tabelaSimulacao: freteSelecionadaDetalhe,
      tabelaRpa: freteSelecionadaDetalhe,
      ...
      taxaAplicadaTexto: detalheTransportadorExcel.taxaAplicadaTexto || '',
      ...
      valorFixoAplicado: Number(freteExcel.valorFixoAplicado || freteExcel.valorFaixa || freteExcel.valorBaseFaixa || 0),
...
function finalizarSimulacaoRealizado(estado, { transportadoraSelecionada = '' } = {}) {
  const {
    rotasMap, cotacaoMap, transportadorasMap, ufsMap, pedidosGanhariaSet, pedidosRetidosSet, pedidosCapturadosSet, pedidosComTabelaSet, pedidosTotaisSet, ctesDetalhes, ctesAjusteRotaExcel, diagnostico, dias, meses,
    ctesAnalisados, ctesSimulados, ctesComTabelaSelecionada, ctesGanhariaSelecionada,
    ctesPerdidosSelecionada, ctesSemTabelaSelecionada, ctesSemTabelaGeral, freteRealizado,
    freteRealizadoComTabelaSelecionada, freteSelecionada, freteVencedor, valorNF,
    valorNFComTabelaSelecionada, peso, volumes, cubagemTotal, pesoComTabelaSelecionada,
  ...
  const faturamentoProjetadoNegociacao = freteSelecionadaRetida + freteCapturadoTabela;
  const freteBaseProjetadoNegociacao = freteRealizadoRetidoSelecionada + freteCapturadoRealizado;
  const savingProjetadoNegociacao = Math.max(freteBaseProjetadoNegociacao - faturamentoProjetadoNegociacao, 0);
  const faturamentoProjetadoNegociacaoMes = meses ? faturamentoProjetadoNegociacao / meses : faturamentoProjetadoNegociacao;
  const faturamentoProjetadoNegociacaoAno = faturamentoProjetadoNegociacaoMes * 12;
  const savingProjetadoNegociacaoMes = meses ? savingProjetadoNegociacao / meses : savingProjetadoNegociacao;
  const savingProjetadoNegociacaoAno = savingProjetadoNegociacaoMes * 12;
  ...
  const resumo = {
    ...
    volumes,
    cubagemTotal,
    pesoComTabelaSelecionada,
    volumesComTabelaSelecionada,
    cubagemComTabelaSelecionada,
    linhasComTracking,
    dias,
    meses,
    ...
    pedidosComTabelaSelecionada: pedidosComTabelaSet?.size || 0,
    ...
    savingProjetadoNegociacao,
    savingProjetadoNegociacaoMes,
    savingProjetadoNegociacaoAno,
...
export default function SimuladorPage({ transportadoras = [] }) {
  ...
  const [biTransportadorasRealizado, setBiTransportadorasRealizado] = useState([]); // preenchido = modo seleção manual
  const [biTransportadorasExcluidasRealizado, setBiTransportadorasExcluidasRealizado] = useState([]); // preenchido = Todas, exceto...
  ...
  const limparFiltrosBiRealizado = () => {
    ...
    setBiTransportadorasExcluidasRealizado([]);
  ...
  const toggleBiTransportadoraRealizado = (nome) => {
    ...
    // Regra UX 4.21b:
    // - Quando está em "Todas", clicar em uma transportadora DESMARCA/EXCLUI aquela transportadora.
    // - Quando está em seleção manual, clicar alterna a seleção manual.
    // Isso evita precisar marcar todas as transportadoras só para retirar uma ou duas.
    if (biTransportadorasRealizado.length) {
      setBiTransportadorasExcluidasRealizado([]);
      setBiTransportadorasRealizado((selecionadasAtuais) => (
        selecionadasAtuais.includes(nome)
          ? selecionadasAtuais.filter((item) => item !== nome)
          : [...selecionadasAtuais, nome]
      ));
      return;
    }

    setBiTransportadorasExcluidasRealizado((excluidasAtuais) => (
      excluidasAtuais.includes(nome)
  ...
  // Sugestão de origem: usa só negociação + cadastro oficial vivo do Supabase.
  // Não usa a "grade" local (prop `transportadoras`, cache que pode ficar meses
  // desatualizada) — ela já causou sugestão de cidade que a transportadora não
  // atende mais (cadastro antigo divergente do Supabase atual).
  const basesMalhaRealizadoAutoritativas = useMemo(() => {
    if (!transportadoraRealizado) return [];
    const bases = [];
    const negociacao = transportadorasNegociacaoRealizado.find((item) => item.nome === transportadoraRealizado);
    if (negociacao) bases.push(negociacao);
    ...
    if (oficiaisCarregadas.length) bases.push(...oficiaisCarregadas);
    return mesclarBasesTransportadorasSimulador(bases);
  }, [transportadoraRealizado, transportadorasNegociacaoRealizado, baseOficialRealizadoSelecionada]);

  const origensMalhaRealizadoDisponiveis = useMemo(
    () => extrairOrigensBaseSimulador(basesMalhaRealizadoAutoritativas, canalRealizado),
    [basesMalhaRealizadoAutoritativas, canalRealizado]
  );
  ...
  const rowsRealizadoVisiveis = useMemo(() => {
    ...
    } else if (biTransportadorasExcluidasRealizado.length) {
      const excluidas = new Set(biTransportadorasExcluidasRealizado.map((nome) => normalizarTransportadoraSimulador(nome)));
      linhas = linhas.filter((row) => !excluidas.has(normalizarTransportadoraSimulador(row.transportadora)));
    }
    ...
    biTransportadorasExcluidasRealizado,
    biOrigemRealizado,
    biDestinoRealizado,
    biUfOrigemRealizado,
  ...
  const negociacaoResultadoRealizado = useMemo(() => {
    const negociacaoId = resultadoRealizado?.negociacaoId;
    if (!negociacaoId) return null;

    return negociacoesSimulador.find((tabela) => tabela.id === negociacaoId)
      || resultadoRealizado.negociacao
      || null;
  }, [negociacoesSimulador, resultadoRealizado]);

  const salvarResultadoNegociacaoRealizado = async () => {
    if (!resultadoRealizado) return;

    const negociacaoId = resultadoRealizado.negociacaoId;
    if (!negociacaoId) {
      setErroSimulacao('Este resultado não está vinculado a uma negociação. Busque os CT-es e simule novamente selecionando a negociação correta.');
      return;
    }
    ...
    try {
      const contextoLaudos = {
        ...
        tipoNegociacao: resultadoRealizado.tipoNegociacao || resultadoRealizado.filtros?.tipoNegociacao || tipoNegociacaoSimulador(negociacaoResultadoRealizado),
      ...
      const negociacaoAtualizada = await salvarResultadoSimulacaoNegociacao(negociacaoId, {
        ...resultadoRealizado,
  ...
  const origensRealizadoDisponiveis = useMemo(() => {
    // Sem transportadora/tabela selecionada: sugestão genérica (canal ou tudo).
    if (!transportadoraRealizado) {
      const porCanal = opcoesOnline.origensPorCanal?.[canalRealizado];
      return porCanal?.length ? porCanal : todasOrigens;
    }

    if (origensMalhaRealizadoDisponiveis.length) return origensMalhaRealizadoDisponiveis;

    // Match por nome tolerante a acento/maiúscula/espaço — a chave em
    // opcoesOnline.origensPorTransportadora vem direto do cadastro (nome exato),
    // que pode não bater 100% com o valor selecionado no campo.
    const chaveNorm = normalizarTransportadoraSimulador(transportadoraRealizado);
    const entrada = Object.entries(opcoesOnline.origensPorTransportadora || {})
      .find(([nome]) => normalizarTransportadoraSimulador(nome) === chaveNorm);
    if (entrada?.[1]?.length) return entrada[1];

    // Nenhuma origem reconhecida para esta transportadora especificamente: não
    // sugerir origens de outras transportadoras, senão o usuário testa uma
    // cidade que ela não atende e o resultado vem zerado sem explicação.
    return [];
  ...
  const onBuscarCtesRealizado = async () => {
    ...
    try {
      ...
      // o texto selecionado NÃO é já um nome de cadastro oficial conhecido (ex.: veio
      // digitado/colado de fora). Aplicar em cima de um nome oficial já correto
      // (ex.: "3G TRANSPORTE") podia trocá-lo por um apelido cadastrado em outro
      // fluxo (ex.: "3G", criado pelo reajuste) e quebrar o casamento de nome
      // depois — a malha carregava, mas a linha vencedora nunca era reconhecida
      // como a transportadora selecionada.
      const ehTransportadoraOficialConhecida = todasTransportadorasDisponiveis.includes(transportadoraRealizado);
      const nomeTabelaSelecionada = (ehNegociacaoSelecionada || ehTransportadoraOficialConhecida)
  ...
  const onSimularRealizadoBase = async () => {
    ...
    try {
      ...
      // A transportadora base identifica a negociação. O reajuste disputa todos
      // os CT-es realizados no recorte para mostrar de quem capturaria volume.
      const rowsComIbgeEscopoNegociacao = rowsBase;

      atualizarProcessamentoUi('Aplicando malha da transportadora/tabela...', 40);

      const origensMalha = expandirOrigensOperacionaisSimulador(new Set(
        baseSelecionada
      ...
      if (!baseParaSimulacao.length) {
        ...
        setResultadoRealizado({
          ...
          negociacaoId: negociacaoRealizadoAtual?.id || '',
          negociacaoNome: negociacaoRealizadoAtual?.transportadora || '',
          negociacaoLabel: negociacaoRealizadoAtual ? labelTabelaNegociacaoSimulador(negociacaoRealizadoAtual) : '',
          negociacao: negociacaoRealizadoAtual
            ? {
                id: negociacaoRealizadoAtual.id,
                transportadora: negociacaoRealizadoAtual.transportadora,
                canal: negociacaoRealizadoAtual.canal,
                tipo_negociacao: negociacaoRealizadoAtual.tipo_negociacao,
              }
            : null,
        });
      ...
      const aplicarResultadoSimulacaoRealizado = (resultado) => {
        ...
        filtros: {
          ...
          filtrosBiAplicados: {
            modoTransportadoras: biTransportadorasRealizado.length ? 'selecionadas' : (biTransportadorasExcluidasRealizado.length ? 'todas_exceto' : 'todas'),
            transportadoras: biTransportadorasRealizado,
            transportadorasExcluidas: biTransportadorasExcluidasRealizado,
  ...
  const exportarLaudoAjusteRotaFaixaTransportador = ({ incluirSavingGerencial = false } = {}) => {
    ...
    const { transportadora } = montarDadosAjusteRotaFaixa(resultadoRealizado);
    const html = gerarHtmlAjusteRotaFaixa(resultadoRealizado, { incluirSavingGerencial });
    ...
    const link = document.createElement('a');
    link.href = url;
    link.setAttribute('download', `${nomeBase}.html`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const exportarExcelAjusteRotaFaixaTransportador = ({ incluirSavingGerencial = false } = {}) => {
    if (!resultadoRealizado?.rotasCotacao?.length) return;
    const { transportadora } = montarDadosAjusteRotaFaixa(resultadoRealizado);
    const wb = gerarWorkbookAjusteRotaFaixa(resultadoRealizado, { incluirSavingGerencial });
    const nomeBase = nomeArquivoSeguro(incluirSavingGerencial
        ...
        <div className="sim-card">
          <div className="sim-resultado-topo compact-top">
            ...
            <button className="sim-tab" type="button"
              ...
              disabled={!resultadoRealizado?.ctesAnalisados || !resultadoRealizado?.negociacaoId || salvandoResultadoNegociacao}
              title={resultadoRealizado?.negociacaoLabel
                ? `Salva o resultado em ${resultadoRealizado.negociacaoLabel}`
                : 'O resultado precisa ser gerado a partir de uma tabela em negociação'}
          ...
          {resultadoRealizado?.negociacaoId && (
            <div className="sim-alert info" style={{ marginTop: 10 }}>
              <strong>Destino do salvamento:</strong>{' '}
              {resultadoRealizado.negociacaoLabel || resultadoRealizado.negociacaoNome || resultadoRealizado.negociacaoId}.
              O resultado será salvo na negociação usada quando a base foi buscada e simulada.
            </div>
          )}
                  ...
                  {biTranspRealizadoAberto && (
                    <div style={{ position: 'absolute', zIndex: 60, top: 'calc(100% + 4px)', left: 0, right: 0, maxHeight: 320, overflow: 'auto', background: '#fff', border: '1px solid #cbd5e1', borderRadius: 10, boxShadow: '0 14px 34px rgba(15, 23, 42, 0.18)', padding: 10 }}>
                      <button
                        ...
                        style={{ width: '100%', border: '1px solid #cbd5e1', borderRadius: 8, background: !biTransportadorasRealizado.length && !biTransportadorasExcluidasRealizado.length ? '#dbeafe' : '#f8fafc', color: '#0f172a', padding: '7px 9px', textAlign: 'left', fontWeight: 700, cursor: 'pointer', marginBottom: 8 }}
                      >
                        {!biTransportadorasRealizado.length && !biTransportadorasExcluidasRealizado.length ? '☑' : '☐'} Todas
                        {biTransportadorasExcluidasRealizado.length ? ` (limpar ${biTransportadorasExcluidasRealizado.length} exclusão(ões))` : ''}
                      </button>
                      <div style={{ display: 'grid', gap: 6 }}>
                        {opcoesBiRealizado.transportadoras.map(({ nome, qtd }) => {
                          const modoSelecaoManual = biTransportadorasRealizado.length > 0;
                          const excluida = biTransportadorasExcluidasRealizado.includes(nome);
                          const marcado = modoSelecaoManual ? biTransportadorasRealizado.includes(nome) : !excluida;
                          return (
                            <button
                              key={nome}
                              type="button"
                              onClick={() => toggleBiTransportadoraRealizado(nome)}
                              title={marcado ? 'Clique para desmarcar esta transportadora da simulação' : 'Clique para incluir esta transportadora novamente'}
                              style={{ width: '100%', border: '1px solid #e2e8f0', borderRadius: 8, background: marcado ? '#fff' : '#fee2e2', color: '#0f172a', padding: '7px 9px', textAlign: 'left', fontWeight: marcado ? 500 : 700, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', gap: 8 }}
                            >
          ...
          {resultadoRealizado && (
            <div style={{ marginTop: 18, display: 'grid', gap: 16 }}>
              ...
              {(resultadoRealizado.diagnostico?.vencedorDivergente || []).length > 0 && (
                <div className="sim-alert warning">
                  ⚠️ Em {resultadoRealizado.diagnostico.vencedorDivergente.reduce((soma, [, qtd]) => soma + qtd, 0).toLocaleString('pt-BR')}
                  {' '}CT-e(s), o simulador achou candidato/preço na base carregada, mas de outra transportadora — não de
                  {' '}"{resultadoRealizado.filtros?.transportadora || transportadoraRealizado}":
              ...
              {(() => {
                const ehReajusteResultado = resultadoRealizado.tipoNegociacao === 'REAJUSTE_TABELA_EXISTENTE'
                ...
                const faturamentoProjetado = Number(
                  ehReajusteResultado
                    ? (resultadoRealizado.faturamentoProjetadoNegociacao || resultadoRealizado.freteSelecionadaGanhadora || 0)
                ...
                const savingMensal = Number(
                  ehReajusteResultado
                    ...
                    : (resultadoRealizado.savingSelecionadaVsRealMes || resultadoRealizado.savingProjetadoNegociacaoMes || 0)
                );
                const savingAnual = Number(
                  ehReajusteResultado
                ...
                const volumesProjetados = Number(
                  ehReajusteResultado
                    ...
                    : (resultadoRealizado.volumesGanhariaSelecionada || resultadoRealizado.volumesCapturados || 0)
                );
                const cubagemProjetada = Number(
                  ehReajusteResultado
                    ? ((resultadoRealizado.cubagemRetidaSelecionada || 0) + (resultadoRealizado.cubagemCapturada || 0))
                    : (resultadoRealizado.cubagemGanhariaSelecionada || resultadoRealizado.cubagemCapturada || 0)
                );
                const pesoProjetado = Number(
                  ehReajusteResultado
                    ? ((resultadoRealizado.pesoRetidoSelecionada || 0) + (resultadoRealizado.pesoCapturado || 0))
                    : (resultadoRealizado.pesoGanhariaSelecionada || resultadoRealizado.pesoCapturado || 0)
                );
                ...
                return (
                  <div className="sim-parametros-box" style={{ border: '1px solid #bfdbfe', background: '#f8fbff' }}>
                    <div className="sim-parametros-header">
                      <div>
                        <strong>Resumo executivo da negociação</strong>
                        <p>{ehReajusteResultado ? 'Uma análise com duas leituras: reajuste da carteira atual e desempenho da tabela nova no mercado.' : 'Simulação normal: disputa pura da tabela contra o realizado e concorrentes.'}</p>
                      </div>
                    </div>
                    {ehReajusteResultado && (
                      <>
                        ...
                        <div className="summary-strip" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))' }}>
                          ...
                          <div className="summary-card"><span>{impactoValorReajuste >= 0 ? 'Aumento no período' : 'Redução no período'}</span><strong>{formatMoney(impactoValorReajuste)}</strong><small>{formatPercent(impactoPercentualReajuste)} vs atual</small></div>
                                          ...
                                          {item.vencedorDetalhes && (
                                            <div style={{ background: '#fff', border: '1px solid #bbf7d0', borderRadius: 8, padding: 12 }}>
                                              ...
                                              <div style={{ display: 'grid', gap: 3, fontSize: '0.78rem', color: '#334155' }}>
                                                ...
                                                <div>
                                                  Origem da cubagem:{' '}
                                                  <strong>{item.trackingMatch ? 'Tracking vinculado ao CT-e' : 'sem vínculo com Tracking'}</strong>
                                                </div>
                                                {item.trackingMatch && (
                                                  <div style={{ color: '#475569' }}>
                                                    Chave CT-e: <strong>{item.chaveCte || 'não informada'}</strong>
                                                    {' · '}Vínculo: <strong>{item.trackingOrigemVinculo || 'não identificado'}</strong>
                                                    {' · '}Linha(s) agregada(s): <strong>{item.trackingLinhas || 1}</strong>
                                                    {' · '}Cubagem recebida: <strong>{Number(item.cubagemOriginalTracking || item.cubagem || 0).toFixed(6)} m³</strong>
                                                  </div>
                                                )}
                                                ...
                                                {item.vencedorDetalhes?.frete?.tipoCalculo === 'PERCENTUAL' && (
                                                  <div style={{ padding: '6px 8px', background: '#eff6ff', borderRadius: 6 }}>
                                                    <strong>Maior valor:</strong>{' '}
                                                    kg garantia {formatMoney(item.vencedorDetalhes?.frete?.valorKgGarantia)}
                                                    {' × '}frete % {formatMoney(item.vencedorDetalhes?.frete?.valorPercentualCalculado)}
                                                    {' × '}frete mínimo {formatMoney(item.vencedorDetalhes?.frete?.minimoAplicavel)}
                                                    {' → '}<strong>{formatMoney(item.vencedorDetalhes?.frete?.valorBase)}</strong>
                                                  </div>
                                                )}
                                                {(() => {
