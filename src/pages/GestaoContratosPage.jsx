...
export default function GestaoContratosPage({ sessao }) {
  ...
  const porValor = criterio === 'valor';
  const metrica = (l) => Number((porValor ? l.valor : l.ctes) || 0);
  const fmtMetrica = (n) => porValor ? fmtMoeda(n) : fmt(n);
  const pareto = linhas.filter((l) => l.no_pareto);
  const comContrato = pareto.filter((l) => l.status_contrato === 'vigente').length;
  const ctesPareto = pareto.reduce((s, l) => s + metrica(l), 0);
  const ctesComContrato = pareto.filter((l) => l.status_contrato === 'vigente').reduce((s, l) => s + metrica(l), 0);
  ...
  async function buscarTransportadores() {
    ...
    try {
      const [ranking, pendentes] = await Promise.all([carregarGestaoContratos(competencia, criterio), carregarSemVinculoContratos(competencia)]);
  ...
  async function alternarCriterio(novoCriterio) {
    if (novoCriterio === criterio) return;
    setCriterio(novoCriterio);
    if (!competenciaCarregada) return;
    setCarregando(true); setErro('');
    try {
      const [ranking, pendentes] = await Promise.all([carregarGestaoContratos(competenciaCarregada, novoCriterio), carregarSemVinculoContratos(competenciaCarregada)]);
      setLinhas(ranking); setSemVinculoDetalhes(pendentes);
    } catch (e) { setErro(e.message); } finally { setCarregando(false); }
  }
  ...
  return <div className="contratos-page">
    ...
    <section className="contratos-cards"><article><span>Cobertura selecionada</span><strong>{pct(pareto.at(-1)?.percentual_acumulado)}</strong><small>{fmtMetrica(ctesPareto)} {porValor ? '' : 'CT-es'}</small></article><article><span>Transportadores no 80%</span><strong>{fmt(pareto.length)}</strong><small>nomes canônicos vinculados</small></article><article><span>{porValor ? 'Valor com contrato vigente' : 'CT-es com contrato vigente'}</span><strong>{pct(coberturaContratada)}</strong><small>{fmtMetrica(ctesComContrato)} de {fmtMetrica(ctesPareto)} {porValor ? '' : 'CT-es'} · {comContrato}/{pareto.length} transportadores</small></article><article className={entradas ? 'attention' : ''}><span>Entraram no período</span><strong>{entradas}</strong><small>exigem conferência</small></article></section>
    ...
    <section className="contratos-panel"><div className="contratos-toolbar"><div><h2>Carteira de {mesLabel(competenciaCarregada)}</h2><p>A última transportadora pode fazer a cobertura ultrapassar 80%.</p></div><div className="contratos-periodo-toggle"><button type="button" className={!porValor ? 'active' : ''} onClick={() => alternarCriterio('quantidade')} disabled={carregando}>Por quantidade</button><button type="button" className={porValor ? 'active' : ''} onClick={() => alternarCriterio('valor')} disabled={carregando}>Por valor</button></div><input placeholder="Buscar transportador" value={busca} onChange={(e) => setBusca(e.target.value)} /><label className="contratos-check"><input type="checkbox" checked={soPareto} onChange={(e) => setSoPareto(e.target.checked)} /> Somente 80%</label></div><div className="contratos-explicacao"><div><strong>Participação na carteira</strong><span>{porValor ? 'Valor de frete da transportadora ÷ valor total que forma a carteira dos 80%.' : 'CT-es da transportadora ÷ total de CT-es que formam a carteira dos 80%.'}</span></div><div><strong>Acumulado do ranking</strong><span>Soma sobre o universo total, usada para identificar quando o ranking alcança 80%.</span></div><small>Os CT-es pendentes mostrados no alerta acima ainda estão fora do cálculo.</small></div>
      <div className="contratos-table-wrap"><table className="contratos-table"><thead><tr><th>#</th><th>Transportador (nome da tabela)</th><th>Movimento</th><th>CT-es</th><th>Valor de frete</th><th title="Percentual individual dentro da carteira selecionada de 80%">Participação na carteira</th><th title="Soma sobre o universo total até esta linha">Acumulado até aqui</th><th>Status do contrato</th><th>Início</th><th>Fim</th><th>Observações</th></tr></thead><tbody>{carregando ? <tr><td colSpan="11" className="empty">Calculando a curva de 80%...</td></tr> : visiveis.map((l) => <tr key={l.transportadora} className={l.no_pareto ? 'pareto' : ''}><td>{l.posicao || '—'}</td><td><strong>{l.transportadora}</strong></td><td><span className={`movimento ${l.movimento}`}>{l.movimento === 'entrou' ? '↑ Entrou' : l.movimento === 'saiu' ? '↓ Saiu' : l.movimento === 'permaneceu' ? '• Permaneceu' : 'Fora'}</span></td><td>{fmt(l.ctes)}</td><td>{fmtMoeda(l.valor)}</td><td>{l.no_pareto && ctesPareto ? pct(metrica(l) / ctesPareto * 100) : '—'}</td><td><b>{pct(l.percentual_acumulado)}</b></td><td><select disabled={salvando === l.transportadora} value={l.status_contrato || 'sem_contrato'} onChange={(e) => atualizar(l, 'status', e.target.value)}>{STATUS.map(([v, t]) => <option key={v} value={v}>{t}</option>)}</select></td><td><input type="date" value={l.inicio_vigencia || ''} onChange={(e) => atualizar(l, 'inicio_vigencia', e.target.value)} /></td><td><input type="date" value={l.fim_vigencia || ''} onChange={(e) => atualizar(l, 'fim_vigencia', e.target.value)} /></td><td><input placeholder="Adicionar nota" defaultValue={l.observacoes || ''} onBlur={(e) => { if (e.target.value !== (l.observacoes || '')) atualizar(l, 'observacoes', e.target.value); }} /></td></tr>)}</tbody></table></div>
    </section></>}
  </div>;
}
