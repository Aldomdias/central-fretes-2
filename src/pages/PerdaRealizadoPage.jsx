...
const FILTROS_BI_PADRAO = {
  ...
  soPerda: false,
  calcConfirmado: false, // apenas CTes onde o pago bate com o calculado (±10%) da realizada
  calcVsCalc: false,    // troca valorPago por calculoRealizada.total → perda = calc_realizada - calc_ganhadora
};
...
function passaFiltrosBiDetalhe(d = {}, filtros = {}) {
  ...
  if (filtros.calcConfirmado) {
    const pago = safeNumber(d.valorPago);
    const calc = safeNumber(d.calculoRealizada?.total ?? d.calculoRealizada?.subtotal);
    if (!calc || pago <= 0) return false;
    if (Math.abs(pago - calc) / pago > 0.10) return false;
  }
  return true;
}
...
function aplicarCalcVsCalc(detalhes) {
  return detalhes.map((d) => {
    const calcReal = safeNumber(d.calculoRealizada?.total ?? d.calculoRealizada?.subtotal);
    if (!calcReal) return d;
    const ganhadora = safeNumber(d.valorGanhadora);
    const perda = Math.round((calcReal - ganhadora) * 100) / 100;
    const temPerda = perda > 0.05;
    const perdaPercentual = calcReal > 0 ? (perda / calcReal) * 100 : 0;
    return { ...d, valorPago: calcReal, perda: Math.max(perda, 0), perdaPercentual, temPerda };
  });
}

function recalcularResultadoBi(resultado = {}, filtros = {}) {
  let detalhesBi = (resultado.detalhes || []).filter((d) => passaFiltrosBiDetalhe(d, filtros));
  if (filtros.calcVsCalc) detalhesBi = aplicarCalcVsCalc(detalhesBi);
...
function filtrosBiAtivos(filtros = {}) {
  return Object.entries(filtros).some(([k, v]) => {
    if (k === 'soPerda' || k === 'calcConfirmado' || k === 'calcVsCalc') return Boolean(v);
...
export default function PerdaRealizadoPage() {
  ...
  const [mostrarMesmaTransp, setMostrarMesmaTransp] = useState(false);
  ...
  return <div className="page-shell">
    ...
    <div className="page-header"><span className="amd-mini-brand">Realizado · Análise</span><h1>Perda por Transportadora Mais Cara</h1><p>Compara o frete pago com a opção mais barata ativa disponível nas tabelas. O processamento é feito por lotes de rotas para evitar timeout.</p><BaseCtesStatus /></div>
