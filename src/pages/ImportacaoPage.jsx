...
import { baixarModelo, buildCoberturaReport, buildImportPayload, exportarSecao, parseFileToRows } from '../utils/importacao';
  ...
  return (
    <div className="page-shell">
      ...
      <div className="feature-grid import-grid">
        <div className="panel-card">
          ...
          <div className="toolbar-wrap">
            ...
            <label className="btn-primary inline-upload">
              ...
              <input type="file" accept=".xlsx,.xls,.csv" multiple onChange={handleFiles} hidden />
            </label>
          </div>
          <div className="hint-box top-space">
            <strong>Observações:</strong><br />
            • Rotas e fretes foram adequados ao padrão dos seus arquivos reais.<br />
            • Dentro de cada origem também existe <strong>Exportar</strong>, <strong>Baixar Modelo</strong>, <strong>Importar</strong> e <strong>Excluir Tudo</strong>.<br />
            • Em taxas especiais, <strong>GRIS</strong> e <strong>Ad Valorem</strong> por IBGE têm prioridade sobre as generalidades.
          </div>
        </div>
        ...
        <div className="panel-card">
          ...
          <div className="list-stack compact-list">
            ...
            )) : <div className="empty-note">Nenhum arquivo importado nesta sessão.</div>}
          </div>
        </div>
      </div>
        ...
        <div className="panel-card">
          ...
          {detalhe.erros?.length ? (
            <div className="table-card">
              <table>
                <thead><tr><th>Linha</th><th>Coluna</th><th>Mensagem</th></tr></thead>
                <tbody>
                  {detalhe.erros.slice(0, 50).map((erro, idx) => <tr key={idx}><td>{erro.linha}</td><td>{erro.coluna || 'layout'}</td><td>{erro.mensagem}</td></tr>)}
                </tbody>
              </table>
            </div>
      ...
      <div className="panel-card">
        <div className="tab-panel-header spaced">
          <div>
            <div className="panel-title no-margin">📍 Cobertura por transportadora e origem</div>
            <p>Use este painel para enxergar rápido o que ainda está sem frete importado.</p>
          </div>
          <input className="search-input small-width" placeholder="Filtrar por transportadora ou origem" value={filtro} onChange={(e) => setFiltro(e.target.value)} />
        </div>
        <div className="table-card">
          <table>
            <thead><tr><th>Transportadora</th><th>Origem</th><th>Canal</th><th>Cobertura</th><th>Rotas</th><th>Cotações</th><th>Taxas</th><th>Destinos</th></tr></thead>
            <tbody>
              {pendencias.length ? pendencias.map((item, idx) => (
                <tr key={`${item.transportadora}-${item.origem}-${idx}`}>
                  <td>{item.transportadora}</td><td>{item.origem}</td><td>{item.canal}</td><td><CoberturaBadge value={item.cobertura} /></td><td>{item.totalRotas}</td><td>{item.totalCotacoes}</td><td>{item.totalTaxas}</td><td>{item.destinos}</td>
                </tr>
              )) : <tr><td colSpan="8" className="empty-cell">Nenhum resultado encontrado.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
