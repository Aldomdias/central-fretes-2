...
<div class="portal-links">${listaPortais.map((p) => (
  ...
  const semEntrega = rows.filter((r) => r.entrega_status && r.entrega_status !== "ENTREGUE");
  const blocoEntrega = semEntrega.length
    ? `<div class="entrega-box"><strong>⚠ ${semEntrega.length} CT-e(s) sem entrega comprovada — favor verificar</strong><p>O pagamento da fatura só é liberado quando todos os CT-es estiverem entregues. Envie o comprovante de entrega (canhoto/POD) dos CT-es abaixo:</p><p class="entrega-lista">${semEntrega.map((r) => `${esc(r.numero_cte || r.chave_cte || "-")}${r.tem_fatura && (r.numeros_fatura || []).length ? ` (fatura ${esc(r.numeros_fatura.join(", "))})` : ""}${r.entrega_status === "SEM_TRACKING" ? " — sem rastreamento" : " — não entregue"}`).join(" · ")}</p></div>`
    : "";
  const cssEntrega = `.entrega-box{margin:0 32px 16px;padding:14px 18px;background:#fef2f2;border:1px solid #fca5a5;border-radius:10px;color:#7f1d1d}.entrega-box p{margin:6px 0 0;font-size:13px}.entrega-lista{font-weight:700}`;

  return html
    .replace("</style>", `${cssPortal}${cssEntrega}</style>`)
    .replace('<section class="lista">', `${blocoEntrega}<section class="lista">`)
