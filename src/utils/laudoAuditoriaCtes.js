...
export function gerarHtmlLaudoAuditoriaCtes(
  ...
  const linhas = rows
    .map((r, index) => {
      ...
      return `<tr class="cte-row" onclick="toggleDetail('${id}')"><td>${esc(faturaTexto)}</td><td>${esc(r.numero_cte || "-")}</td><td>${esc(r.transportadora || "-")}</td><td>${esc(`${r.cidade_origem || "-"}/${r.uf_origem || "-"} → ${r.cidade_destino || "-"}/${r.uf_destino || "-"}`)}</td><td>${numero(r.peso)} kg</td><td>${dinheiro(r.valor_cte)}</td><td>${calculadoPublico > 0 ? dinheiro(calculadoPublico) : "Sem cálculo"}</td><td class="${statusPublico === "OK" ? "" : "erro"}">${dinheiro(diferencaPublica)}</td><td><span class="status ${statusPublico === "OK" ? "status-ok" : "status-revisar"}">${statusPublico}</span></td></tr><tr id="${id}" class="detail-row"><td colspan="9"><div class="detail-panel"><div class="detail-title"><strong>Detalhes do cálculo — CT-e ${esc(r.numero_cte || "-")}</strong><span>${esc(r.chave_cte || "")}</span></div><div class="detail-grid">${detalheCte(r, diferenca, ocultarCobrancaAMenor)}</div>${r.motivo_sem_calculo ? `<p class="motivo"><b>Ponto para verificação:</b> ${esc(r.motivo_sem_calculo)}</p>` : ""}<p class="acao"><b>Ação solicitada:</b> verificar a emissão e os valores deste CT-e. Caso a cobrança esteja incorreta e o documento ainda esteja dentro do prazo legal, solicitamos o cancelamento do CT-e e a reemissão correta. Se o cancelamento não for aplicável, favor informar a tratativa adequada.</p></div></td></tr>`;
    })
    .join("");
  ...
  const scriptResumoPublico = scriptResumoDinamico.replace(
    "calculado+=moedaNumero(row.cells[6]&&row.cells[6].textContent);",
    `var calculadoLinha=moedaNumero(row.cells[6]&&row.cells[6].textContent);calculado+=${mostrarCobrancaAMenor ? "calculadoLinha" : "Math.min(calculadoLinha,moedaNumero(row.cells[5]&&row.cells[5].textContent))"};`,
  );
  return html
    ...
    .replace("</script>", `${scriptsFiltros}${scriptExcelDetalhado}${scriptDetalhesEmColunas}${scriptResumoPublico}</script>`);
}
