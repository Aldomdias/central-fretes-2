...
// --- Helpers adicionados para padronização automática da cotação ---
...
export function obterUfDoDestino(ibgeDestino, baseIbge = []) {
  ...
  const listaMunicipios = Array.isArray(baseIbge)
    ? baseIbge
    : Array.isArray(baseIbge?.municipios)
      ? baseIbge.municipios
      : [];

  const item = listaMunicipios.find((registro) => {
    const codigo = String(
...
export function montarCotacaoDaRota(rota = {}, dadosGerais = {}, baseIbge = []) {
  ...
  return montarCotacaoPadrao({ origem, ufDestino, cotacaoBase });
}
