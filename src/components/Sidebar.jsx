  ...
  return (
    <aside className="sidebar-app">
      <div className="brand-box amd-brand-box">
        <div className="amd-brand-logo-wrap"><img src={logoAmd} alt="AMD Log" className="amd-brand-logo" /></div>
        <div className="amd-brand-meta"><div className="amd-brand-title">AMD Log</div><div className="amd-brand-subtitle">Plataforma de Fretes</div></div>
      </div>
      <div className="menu-section">
        <div className="menu-section-title">PRINCIPAL</div>
        {menuPrincipal.map((item) => (
          <button key={item.chave} className={paginaAtual === item.chave ? 'nav-item active' : 'nav-item'} onClick={() => onMudarPagina(item.chave)}>
