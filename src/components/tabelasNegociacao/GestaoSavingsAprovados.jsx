        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 4 }}>
          ...
          {progressoTodas ? (
            <span style={{ fontSize: 12, color: '#64748b' }}>
              {progressoTodas.atual}/{progressoTodas.total} · {progressoTodas.transportadora}
            </span>
          ) : null}
        </div>
      )}
      ...
      {Object.keys(resultados).length > 1 ? (
        <div style={{ marginTop: 14, ...gestaoStyles.tabelaWrap }}>
          <table className="sim-table">
            ...
            <tbody>
              {negociacoesAprovadas.filter((item) => resultados[item.id]).map((item) => {
                ...
                return (
                  <tr key={item.id}>
                    ...
                    <td style={{ fontWeight: 700, color: r.totais.saving >= 0 ? '#087f3f' : '#c1121f' }}>{formatMoney(r.totais.saving)}</td>
                    <td>
                      <button type="button" className="sim-tab" style={{ padding: '3px 8px', fontSize: 11 }} onClick={() => verDetalhe(item)}>
                        Ver detalhe
                      </button>
                    </td>
                  </tr>
            ...
            <tfoot>
              <tr>
                ...
                <td style={{ fontWeight: 800 }}>
                  {formatMoney(negociacoesAprovadas.reduce((acc, item) => acc + (resultados[item.id]?.totais.saving || 0), 0))}
                </td>
                <td></td>
              </tr>
            </tfoot>
            ...
            <div key={item.id} id={`saving-item-${item.id}`} style={gestaoStyles.origemCard}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#64748b' }}>{item.nome}{item.canal ? ` · ${item.canal}` : ''}</div>
