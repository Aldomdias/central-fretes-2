...
function normalizeTransportadora(transportadora = {}) {
  return {
    ...
    origens: Array.isArray(transportadora.origens) ? transportadora.origens.map(normalizeOrigem) : [],
  };
}
...
function mergeImport(prev, payload, tipo) {
  ...
  payload.transportadoras.forEach((item) => {
    let transportadora = next.find((current) => current.nome.toLowerCase() === item.nome.toLowerCase());
    if (!transportadora) {
      transportadora = normalizeTransportadora({ id: nextId(next), nome: item.nome, status: item.status || 'Ativa', origens: [] });
      next.push(transportadora);
    }

    let origem = (transportadora.origens || []).find((current) => current.cidade.toLowerCase() === item.origem.cidade.toLowerCase());
    if (!origem) {
    ...
    if (tipo === 'rotas') {
      const enriched = item.origem.rotas.map((row) => ({ ...row, id: nextId(origem.rotas || []) + Math.random() }));
      origem.rotas = [...(origem.rotas || []), ...enriched];
    }
    if (tipo === 'cotacoes') {
      const enriched = item.origem.cotacoes.map((row) => ({ ...row, id: nextId(origem.cotacoes || []) + Math.random() }));
      origem.cotacoes = [...(origem.cotacoes || []), ...enriched];
    }
    if (tipo === 'taxas') {
      const enriched = item.origem.taxasEspeciais.map((row) => ({ ...row, id: nextId(origem.taxasEspeciais || []) + Math.random() }));
...
export function useFreteStore() {
  const [transportadoras, setTransportadoras] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : clone(initialTransportadoras);
  });
  ...
  const api = useMemo(() => ({
    ...
    salvarGeneralidades(transportadoraId, origemId, generalidades) {
      setTransportadoras((prev) => prev.map((t) => t.id !== transportadoraId ? t : ({
        ...t,
        origens: t.origens.map((o) => o.id !== origemId ? o : ({ ...o, generalidades })),
      })));
    },
    salvarOrigem(transportadoraId, origem) {
      setTransportadoras((prev) => prev.map((t) => {
        ...
        const origens = exists
          ? t.origens.map((o) => o.id === origem.id ? normalizeOrigem(origem) : o)
    ...
    salvarTransportadora(transportadora) {
      setTransportadoras((prev) => {
        ...
        return exists ? prev.map((item) => item.id === transportadora.id ? normalized : item) : [...prev, normalized];
      });
    },
    ...
    salvarLinha(transportadoraId, origemId, secao, linha) {
      setTransportadoras((prev) => prev.map((t) => t.id !== transportadoraId ? t : ({
        ...
        origens: t.origens.map((o) => {
          ...
          return {
            ...o,
            [secao]: exists ? lista.map((item) => item.id === linha.id ? linha : item) : [...lista, linha],
          };
        }),
      })));
    },
    removerLinha(transportadoraId, origemId, secao, linhaId) {
      setTransportadoras((prev) => prev.map((t) => t.id !== transportadoraId ? t : ({
        ...t,
        origens: t.origens.map((o) => o.id !== origemId ? o : ({
          ...o,
