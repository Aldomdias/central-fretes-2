-- ============================================================
-- RPC: buscar_base_simulacao
-- Substitui o waterfall de 6-8 chamadas HTTP do JS por 1 chamada
-- Rode no SQL Editor do Supabase APÓS os índices (01_PERFORMANCE_INDEXES.sql)
-- ============================================================
...
BEGIN
  ...
  v_origem_norm := lower(trim(p_origem));

  -- 1. Busca origens filtradas
  SELECT jsonb_agg(o)
  INTO v_origens
  FROM public.origens o
  JOIN public.transportadoras t ON t.id = o.transportadora_id
  WHERE
    (v_origem_norm = '' OR lower(o.cidade) LIKE v_origem_norm || '%' OR lower(o.cidade) LIKE '%' || v_origem_norm || '%')
    AND (array_length(v_canal_cats, 1) IS NULL OR o.canal = ANY(v_canal_cats) OR o.canal IS NULL)
    AND (p_transportadora = '' OR lower(t.nome) LIKE lower(p_transportadora) OR lower(t.nome) LIKE '%' || lower(p_transportadora) || '%');
  ...
  -- Origens que têm pelo menos uma rota
  SELECT array_agg(DISTINCT (r->>'origem_id')::uuid)
  INTO v_origens_com_rota
  FROM jsonb_array_elements(v_rotas) r;

  -- Transportadoras dessas origens
  SELECT array_agg(DISTINCT (o->>'transportadora_id')::uuid)
  INTO v_transp_ids
  FROM jsonb_array_elements(v_origens) o
  WHERE (o->>'id')::uuid = ANY(v_origens_com_rota);

  -- 3. Busca em paralelo: transportadoras, generalidades, cotacoes, taxas
  SELECT jsonb_agg(t)
  INTO v_transportadoras
  FROM public.transportadoras t
  WHERE t.id = ANY(v_transp_ids);

  SELECT jsonb_agg(g)
  INTO v_generalidades
  FROM public.generalidades g
  WHERE g.origem_id = ANY(v_origens_com_rota);

  SELECT jsonb_agg(c)
  INTO v_cotacoes
  FROM public.cotacoes c
  WHERE c.origem_id = ANY(v_origens_com_rota);
  ...
  WHERE tx.origem_id = ANY(v_origens_com_rota)
    AND (
      v_destinos_ibge IS NULL
      OR array_length(v_destinos_ibge, 1) = 0
...
-- Permissão de acesso via anon key
GRANT EXECUTE ON FUNCTION public.buscar_base_simulacao(text, text, text[], text, text) TO anon, authenticated;

-- Teste rápido (substitua pelos valores reais):
-- SELECT public.buscar_base_simulacao('Itajai', 'ATACADO', '{}', 'MG', '');
-- SELECT public.buscar_base_simulacao('', '', '{}', '', 'MOVVI');
