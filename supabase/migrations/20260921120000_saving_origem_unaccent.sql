...
-- Bug: p_origem chegava acentuado (ex.: "ITAJAÍ") do front, mas
-- cidade_origem em realizado_local_ctes é gravada sem acento ("ITAJAI").
-- A comparação `upper(cidade_origem) like upper(p_origem) || '%'` nunca
-- batia, então o saving pós-aprovação e o primeiro CT-e voltavam vazios
-- (0 rotas comparáveis, saving R$0,00) mesmo com CT-es existindo pra
-- transportadora/origem. O front já foi corrigido para limpar o acento
-- antes de mandar o parâmetro; aqui aplicamos unaccent() dos dois lados
-- também, pra blindar qualquer outro chamador que não limpe a origem.
...
with parametros as (
  ...
  where r.data_emissao >= p_data_corte::timestamp
    ...
    and (coalesce(trim(p_origem), '') = '' or upper(unaccent(r.cidade_origem)) like upper(unaccent(trim(p_origem))) || '%')
